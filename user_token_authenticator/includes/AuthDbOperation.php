...
class AuthDbOperation
{
    ...
    function isTokenValid($userType,$userNtcode,$userToken) {
	    $RuserType = '';
	    if($userType=='patient'){
		    $RuserType = 'users';
	    }else{
		    $RuserType = 'doctors_table';
	    }
	    $stmt = $this->con->prepare("SELECT id FROM $RuserType WHERE national_code = '$userNtcode' AND apikey = '$userToken'");
        $stmt->execute();
        $stmt->store_result();
